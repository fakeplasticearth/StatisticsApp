...
public:
	...
	*/
	...
	/*!
	* Функция, возвращающая структуру типа point_th (значение случайной величины и вероятность) по индексу массива.
	* \param index Индекс массива th_points_sorted, принимает значения от 0 до n - 1, где n - размер массива значений случайной величины.
	*/
	point_th get_th_point(int index);
	/*!
	* Функция, которая объединяет частоты для применимости критерия Хи-квадрат.
	*/
	void merge_freqs();
	/*!
	* Функция, возвращающая число степеней свободы.
	* \details Если число степеней свободы не вычислено ранее, вызывается функция merge_freqs(), где вычисляется число степеней свободы.
	*/
	int get_df();
	/*!
	* Функция, вычисляющая статистику критерия Хи-квадрат.
	*/
	void calc_chi2();
	/*!
	* Функция, возвращающая статистику критерия Хи-квадрат.
	*/
	double get_chi2();
	/*!
	* Функция, возвращающая p-value.
	* \details Если p-value не было ранее вычислено, то оно вычисляется и возвращается.
	*/
	double get_pvalue();

	/*!
	* Функция, возвращающая размер распределения
	*/
	int get_size() const;
	/*!
	* Функция, возвращающая максимальное значение случайной величины.
	* \details Эта функция необходима для построения гистограммы.
	*/
	double get_hist_max_value() const;
	/*!
	* Функция, возвращающая минимальное значение случайной величины.
	* \details Эта функция необходима для построения гистограммы.
	*/
	double get_hist_min_value() const;
	/*!
	* Функция, возвращающая модуль минимальной разности между значениями случайной величины.
	* \details Эта функция необходима для построения гистограммы.
	*/
	double get_hist_min_dif_module() const;
	/*!
	* Функция, возвращающая максимальную частоту.
	* \details Эта функция необходима для построения гистограммы.
	*/
